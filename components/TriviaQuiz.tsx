...
import { BrainCircuit, Play, RotateCcw, Trophy, Clock, CheckCircle, Eye, EyeOff, Trash2 } from 'lucide-react';
...
export const TriviaQuiz: React.FC<TriviaQuizProps> = ({ channelConnected }) => {
  ...
  const [roundWinners, setRoundWinners] = useState<string[]>([]); // Users who got current Q right
  ...
  const roundWinnersRef = useRef<string[]>([]);
  ...
  useEffect(() => { roundWinnersRef.current = roundWinners; }, [roundWinners]);
  ...
  useEffect(() => {
    ...
    const cleanup = chatService.onMessage((msg) => {
       ...
       if (isCorrect) {
          ...
          setRoundWinners(prev => [...prev, user]);
          
          // Add Score (Speed Bonus: More time left = more points)
          // Hard mode gets 2x points
      ...
      <div className="w-full h-full flex flex-col items-center justify-center p-6 relative overflow-hidden bg-[#0b0e0f]">
         ...
         {(gameState === 'PLAYING' || gameState === 'REVEAL') && currentQuestion && (
             <div className="w-full max-w-4xl z-10 flex flex-col gap-6">
                 ...
                 {gameState === 'REVEAL' && roundWinners.length > 0 && (
                     <div className="text-center animate-in slide-in-from-bottom-4">
                        <span className="text-green-400 font-bold text-sm bg-green-900/20 px-4 py-2 rounded-full border border-green-500/20">
                           {roundWinners.length} لاعبين أجابوا بشكل صحيح!
                        </span>
                     </div>
                 )}
             </div>
