...
import { Disc, RotateCcw, Skull } from 'lucide-react';
             ...
             <div className="text-center opacity-60">
                ...
                <div className="mt-8 flex gap-4 justify-center">
                   {Array.from({length: 6}).map((_, i) => (
                      <div key={i} className={`w-12 h-12 rounded-full border-2 border-white/20 flex items-center justify-center ${participants[i] ? 'bg-kick-green text-black font-bold' : 'bg-transparent'}`}>
                         {participants[i] ? participants[i].username.charAt(0) : '?'}
                      </div>
                   ))}
                </div>
